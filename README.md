...
- [x] 컴퓨터가 3개의 수를 만들어낸다.
    - 1부터 9까지 서로 다른 3개의 수
- [x] 사용자에게 3개의 수를 입력받는다.
    - 숫자가 아닌 경우 예외 처리
    - 중복된 숫자가 있는 경우 예외 처리
    - 세 개의 수가 아닌 경우 예외 처리
    - 1부터 9까지의 수가 아닌 경우 예외 처리
- [x] 게임 결과를 출력한다.
    - 같은 자리에 있으면 스트라이크
    - 다른 자리에 있으면 볼
    - 같은 수가 전혀 없으면 낫싱
- [x] 컴퓨터가 선택한 3개의 수를 모두 맞히면 게임이 종료된다.
- [x] 사용자로부터 재시작, 종료 입력을 받는다.
    - 숫자를 입력하지 않은 경우 예외 처리
    - 1,2 외의 숫자를 입력한 경우 예외 처리
- [x] 사용자가 1을 입력하면 재시작/ 2를 입력하면 게임 종료 한다.
...
- 값을 하드 코딩하지 않는다
- 구현 순서도 코딩 컨벤션이다. 클래스는 프로퍼티, init 블록, 부 생성자, 메서드, 동반 객체 순으로 작성한다.
